...
use WPPayForm\App\Services\ShortCodeParser;

class GlobalNotificationManager
{
    public function globalNotify($insertId, $formId, $formData, $formattedElements)
    {
        ...
        foreach ($enabledFeeds as $feed) {
            ...
            $processedValues = ShortCodeParser::parse($processedValues, $formId, $entry);
    ...
    public function cleanUpPassword($insertId, $passwordKeys)
    {
        ...
        foreach ($passwordKeys as $passwordKey) {
            if (!empty($formattedData[$passwordKey])) {
                ...
                $truncate = str_repeat("*", strlen($originalPassword)).' '. __('(truncated)', 'wppayform');
