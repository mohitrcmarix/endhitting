...
class RazorpaySettings
{
    ...
    {
        $settings = get_option('wppayform_razorpay_payment_settings', array());
        $defaults = array(
            'payment_mode' => 'test',
            'checkout_type' => 'modal',
            'live_pub_key' => '',
    ...
    public function savePaymentSettings($request)
    {
        ...
        $data = array(
            ...
            'notification' => Arr::get($settings, 'notification')
        );

        do_action('wppayform/before_save_razorpay_settings', $data);
        update_option('wppayform_razorpay_payment_settings', $data, false);
        do_action('wppayform/after_save_razorpay_settings', $data);
