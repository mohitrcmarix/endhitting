...
use WPPayForm\App\Modules\Pro\GateWays\Mollie\API\IPN;
...
class MollieProcessor
{
    ...
    {
        $isLive = (new MollieSettings())->isLive($formId);
    ...
    public function handleRedirect($transaction, $submission, $form, $methodSettings)
    {
        ...
        wp_send_json_success([
            ...
            'message'      => __('You are redirecting to mollie.com to complete the purchase. Please wait while you are redirecting....', 'wppayform'),
        ], 200);
    }
    ...
    public function markAsPaid($status, $updateData, $transaction)
    {
        ...
        SubmissionActivity::createActivity(array(
            ...
            'type' => 'info',
            'created_by' => 'PayForm Bot',
            'content' => sprintf(__('Transaction Marked as paid and Mollie Transaction ID: %s', 'wppayform'), $updateDate['charge_id'])
        ));
    ...
    public function validateSubscription($paymentItems)
    {
        wp_send_json_error(array(
            'message' => __('Mollie doesn\'t support subscriptions right now', 'wppayform'),
            'payment_error' => true
        ), 423);
    }
}
