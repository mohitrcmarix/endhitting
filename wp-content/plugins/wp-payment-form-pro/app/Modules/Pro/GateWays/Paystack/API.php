...
class API
{
    ...
    protected function handleIpn($data)
    {
        ...
        do_action('wppayform_ipn_razorpay_action_' . $status, $submission, $vendorTransaction, $data);

        if ($refundAmount = Arr::get($vendorTransaction, 'amountRefunded.value')) {
            $refundAmount = intval($refundAmount * 100); // in cents
            do_action('wppayform_ipn_razorpay_action_refunded', $refundAmount, $submission, $vendorTransaction, $data);
        }
    }
    ...
    public function makeApiCall($path, $args, $formId, $method = 'GET')
    {
        ...
        } elseif (empty($responseData['status']) && !$responseData['status']) {
            // this is an error
