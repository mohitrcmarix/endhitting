...
class CustomerEmailComponent extends BaseComponent
{
    ...
    public function validateEmailOnSubmission($error, $elementId, $element, $data)
    {
        ...
        if ($value) {
            ...
            if (!is_email($value)) {
                return __('Valid email address is required for field:', 'wppayform') . ' ' . Arr::get($element, 'label');
            }
        }
        ...
        if (Arr::get($element, 'options.confirm_email') == 'yes') {
            ...
            if ($confirmEmailvalue != $value) {
                return Arr::get($element, 'label') . ' & ' . Arr::get($element, 'options.confirm_email_label') . __(' does not match', 'wppayform');
            }
        }
