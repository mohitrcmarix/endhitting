...
class Debug
{
    ...
    public static function getTheme()
    {
        ...
        return array(
            'name' => array(
                'label' => __('Theme Name', 'wppayform'),
                'result' => $theme->Name
            ),
            'version' => array(
                'label' => __('Theme Version', 'wppayform'),
                'result' => $theme->version
            ),
            'author' => array(
                'label' => __('Author', 'wppayform'),
                'result' => $theme->author,
            ),
            'URI' => array(
                'label' => __('Theme URI', 'wppayform'),
    ...
    public static function getServerInfos()
    {
        ...
        if ($wpdb->use_mysqli) {
            ...
            $mysql = __('Cannot connect to MySQL database.', 'wppayform');
        }
        ...
        $serverInfos = array(
            'host' => array(
                'label' => __('Web Server', 'wppayform'),
                'result' => $host,
            ),
            'php_version' => array(
                'label' => __('PHP Version', 'wppayform'),
                'result' => PHP_VERSION,
            ),
            'server_timezone' => array(
                'label' => __('Server Timezone', 'wppayform'),
                'result' => date_default_timezone_get(),
            ),
            'mysql_version' => array(
                'label' => __('MySQL Version', 'wppayform'),
                'result' =>  $mysqlStatus ? $mysql . '(Ok)': $mysql . ('old, issue'),
                'status' => $mysqlStatus ? 'ok' : 'warn'
            ),
            'display_errors' => array(
                'label' => __('Display Errors', 'wppayform'),
                'result' => (ini_get('display_errors')) ? __('Yes', 'wppayform') . ' (' . ini_get('display_errors') . ')(issue)' : 'No Display Errors',
                'status' => ini_get('display_errors') ? 'warn' : 'ok'
            ),
            'memoryLimit' => array(
                'label' => __('Server Memory Limit (PHP)', 'wppayform'),
                'result' => $memoryLimit ? $memoryLimit : '-',
            ),
            'upload_max_filesize' => array(
                'label' => __('Upload Max Filesize', 'wppayform'),
                'result' => $uploadedFileSIze ? $uploadedFileSIze : '-',
            ),
            'mbstring' => array(
                'label' => 'mbstring (' . __('MultiByte String', 'wppayform') . ') ' . __('Enabled', 'wppayform'),
                'result' => extension_loaded('mbstring') ? __('Yes', 'wppayform') : __('Not Enabled (issue)', 'wppayform'),
                'status' => extension_loaded('mbstring') ? 'ok' : 'warn'
            ),
            'fsockopen' => array(
                'label' => 'fsockopen',
                'result' => function_exists('fsockopen') ? __('Yes', 'wppayform') : __('Not', 'wppayform'),
            ),
            'soap' => array(
                'label' => 'SOAP',
                'result' => class_exists('SoapClient') ? __('Yes', 'wppayform') : __('No', 'wppayform'),
            ),
            'post_max_size' => array(
                'label' => __('Post Max Size', 'wppayform'),
                'result' => $maximumPostSize ? $maximumPostSize : '-',
            ),
            'max_execution_time' => array(
                'label' => __('Max Execution Time', 'wppayform'),
                'result' => $executionLimit ? $executionLimit : '-',
            ),
            'max_input_vars' => array(
                'label' => __('Max Input Vars', 'wppayform'),
                'result' => $inputsLimit ? $inputsLimit : '-',
            ),
            'wp_remote_get' =>$remoteGet,
            'wp_remote_post' => $remotePost,
            'curl_init' => array(
                'label' => __('cURL Enabled', 'wppayform'),
                'result' => function_exists('curl_init') ? __('Yes', 'wppayform') : __('Not Enabled (issue)', 'wppayform'),
                'status' => function_exists('curl_init') ? 'ok' : 'warn'
            ),
            'curl_version' => array(
                'label' => __('cURL Version', 'wppayform'),
                'result' => $curlVersion,
            )
        );
    ...
    private static function getWPInfos()
    {
        ...
        $wpInfos = array(
            'site_name' => array(
                'label' => __('Site Name', 'wppayform'),
                'result' => get_bloginfo('name')
            ),
            'home_url' => array(
                'label' => __('Site Homepage', 'wppayform'),
                'result' => home_url()
            ),
            'site_url' => array(
            ...
            'wp_version' => array(
                'label' => __('WP Version', 'wppayform'),
                'result' => $wp_version
            ),
            'wp_debug' => array(
                'label' => __('WP Debug', 'wppayform'),
                'result' => defined('WP_DEBUG') ? (WP_DEBUG ? 'yes' : WP_DEBUG) : 'Not defined'
            ),
            'wp_debug_log' => array(
                'label' => __('WP Debug log', 'wppayform'),
                'result' => defined('WP_DEBUG_LOG') ? (WP_DEBUG_LOG ? 'yes' : WP_DEBUG_LOG) : 'Not defined'
            ),
            'locale' => array(
                'label' => __('Locale', 'wppayform'),
                'result' => get_locale()
            ),
            'multisite' => array(
                'label' => __('Multisite', 'wppayform'),
                'result' => is_multisite() ? 'Yes' : 'No'
            ),
            'memory_limit' => array(
                'label' => __('WP Memory Limit', 'wppayform'),
    ...
    private static function getRequestTest()
    {
        ...
        return array(
                'label' => __('WP Remote GET', 'wppayform'),
    ...
    private static function postRequestTest()
    {
        ...
        return array(
            'label' => __('WP Remote POST', 'wppayform'),
