...
//TODO
/*
* need to make sure that we show a warning on install that let's people know that installing this plugin is irreversible (in other words they can't go from multiple series back to PublishPress Series Core.
*/
...
class osMulti {
	...
	function __construct() {
		...
		add_action('delete_'.ppseries_get_series_slug().'', array(&$this, 'delete_series'), 10, 2);

		//hooking into PublishPress Series
	...
	function update($version) {
		...
		if ( $version == '0.1' || empty($version) ) {
			...
			if ( empty($import_chk) || !$import_chk ) {
				//this means we need to do an import to convert the SERIES_PART meta_key's in PublishPress Series core to the SERIES_PART key for multiples (not reversible!)
	...
	function ajax_series() {
		...
		if ( !isset($response['error'] ) ) {
			$response = array(
				'id' => $series_id,
				'html' => "\n<li id='series-{$series_id}' class='series-added-indicator'>"."<div class='series-li-part'><div><label for='in-series-{$series_id}' class='selectit'>" . '<input value="'. $series_id . '" type="checkbox" name="post_series[]" id="in-series-'. $series_id . '" checked /><input type="hidden" name="is_series_save" value="1" /><span class="li-series-name">' . $series_name . '</span></label></div><div>'.'<span class="to_series_part"><input id="series-part-'.$series_id.'" type="text" class="series_part" size="3" value="" name="series_part['.$series_id.']" /></span></div></div></li>',
				'new_nonce' => $new_nonce,
				'error' => false
				);
		}
	...
	function meta_box() {
		...
		foreach ($posttypes as $posttype) {
			...
			add_meta_box('newseriesdiv', __('Series', 'publishpress-series-pro'), array($this, 'add_meta_box'),
				$posttype, 'side');
		}
	}
	...
	function add_meta_box() {
		...
		<div class="ppseries-title-container">
			<span class="series-title"><?php _e('Series', 'publishpress-series-pro'); ?></span>
			<span class="series-part"><?php _e('Part', 'publishpress-series-pro'); ?></span>
		</div>
		<div class="clear"></div>
        ...
        <div class="series-part-wrap"></div>
			<strong> <?php _e('Post title in widget:', 'publishpress-series-pro'); ?></strong>
			...
			if ( $series = get_the_series($id, false) ) {
				...
				foreach ( $series as $ser ) {
					...
					if ( in_array( $post_status, array( 'publish', 'private' ) ) ) {
						$column_content .= sprintf(__('<a href="%1$s" title="%2$s">%3$s</a> %4$s of %5$s%6$s', 'publishpress-series-pro'), $series_link, $series_name, $series_name, $series_part, $count, $drafts_included);

					} else {
						$column_content .= sprintf(__('<a href="%1$s" title="%2$s">%3$s</a> (Currently as %4$s)', 'publishpress-series-pro'), $series_link, $series_name, $series_name, $series_part);
					}
